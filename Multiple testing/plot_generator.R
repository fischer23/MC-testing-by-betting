...
# Prepare the data
results_df_1_power <- data.frame(
  idx_1 = seq(0.1, 0.9, 0.1), power_BH = power_BH, power_BH_smallB = power_BH_smallB,
  power_bm = power_bm, power_bc = power_bc, power_agg = power_agg, power_amt = power_amt
)

results_df_1_nperm <- data.frame(
  idx_1 = seq(0.1, 0.9, 0.1), nperm_mean_bm = nperm_mean_bm, nperm_mean_bc = nperm_mean_bc,
...
cols <- c(
  "Permutation p-value" = "cornflowerblue", "Permutation p-value (B=200)" = "purple", "Aggressive (ours)" = "red", "Anytime-valid BC (ours)" = "limegreen",
...
p1 <- ggplot(results_df_1_long, aes(x = idx_1, y = power)) +
  ...
  geom_text_repel(
    data = dplyr::filter(results_df_1_long, idx_1 == 0.9),
  ...
  theme(
    legend.position = "none", panel.background = element_blank(), panel.grid.major = element_blank(),
...
# Prepare the data
results_df_2_power <- data.frame(
  idx_2 = c(0.01, 0.05, 0.1, 0.15, 0.2), power_BH = power_BH, power_BH_smallB = power_BH_smallB,
  power_bm = power_bm, power_bc = power_bc, power_agg = power_agg, power_amt = power_amt
)

results_df_2_nperm <- data.frame(
  idx_2 = c(0.01, 0.05, 0.1, 0.15, 0.2), nperm_mean_bm = nperm_mean_bm, nperm_mean_bc = nperm_mean_bc,
  nperm_mean_agg = nperm_mean_agg, nperm_mean_BH = 10000, nperm_mean_BH_smallB = 200, nperm_mean_amt = nperm_mean_amt
)
...
p3 <- ggplot(results_df_2_long, aes(x = idx_2, y = power)) +
  ...
  geom_point(aes(colour = Strategy, shape = Strategy)) +
  geom_text_repel(
    data = dplyr::filter(results_df_2_long, idx_2 == 0.2),
...
# Prepare the data
results_df_3_power <- data.frame(
  idx_3 = c(1, 1.5, 2, 2.5, 3, 3.5, 4), power_BH = power_BH, power_BH_smallB = power_BH_smallB,
  power_bm = power_bm, power_bc = power_bc, power_agg = power_agg, power_amt = power_amt
)

results_df_3_nperm <- data.frame(
  idx_3 = c(1, 1.5, 2, 2.5, 3, 3.5, 4), nperm_mean_bm = nperm_mean_bm, nperm_mean_bc = nperm_mean_bc,
  nperm_mean_agg = nperm_mean_agg, nperm_mean_BH = 10000, nperm_mean_BH_smallB = 200, nperm_mean_amt = nperm_mean_amt
)
...
p5 <- ggplot(results_df_3_long, aes(x = idx_3, y = power)) +
  ...
  geom_point(aes(colour = Strategy, shape = Strategy)) +
  geom_text_repel(
    data = dplyr::filter(results_df_3_long, idx_3 == 4),
...
# Prepare the data
results_df_4_power <- data.frame(
  idx_4 = c(100, 500, 1000, 3000, 5000, 10000), power_BH = power_BH, power_BH_smallB = power_BH_smallB,
  power_bm = power_bm, power_bc = power_bc, power_agg = power_agg, power_amt = power_amt
)

results_df_4_nperm <- data.frame(
  idx_4 = c(100, 500, 1000, 3000, 5000, 10000), nperm_mean_bm = nperm_mean_bm, nperm_mean_bc = nperm_mean_bc,
  nperm_mean_agg = nperm_mean_agg, nperm_mean_BH = 10000, nperm_mean_BH_smallB = 200, nperm_mean_amt = nperm_mean_amt
)
...
p7 <- ggplot(results_df_4_long, aes(x = idx_4, y = power)) +
  ...
  geom_point(aes(colour = Strategy, shape = Strategy)) +
  geom_text_repel(
    data = dplyr::filter(results_df_4_long, idx_4 == 10000),
...
results_df_5 <- data.frame(
  idx_5 = c(0, 0.1, 0.3, 0.5, 0.7, 0.9), FDR_BH = FDR_BH, FDR_BH_smallB = FDR_BH_smallB,
...
p9 <- ggplot(results_df_5, aes(idx_5)) +
  ...
  scale_shape_manual(
    name = "Strategy", values = c("1" = 6, "2" = 3, "3" = 0, "4" = 8, "5" = 1, "6" = 4),
    labels = c("1" = lab[3], "2" = lab[4], "3" = lab[1], "4" = lab[2], "5" = lab[5])
  ) +
  scale_colour_manual(
    name = "Strategy", values = c("1" = col[1], "2" = col[2], "3" = col[3], "4" = col[4], "5" = col[5], "6" = col[6]),
    labels = c("1" = lab[3], "2" = lab[4], "3" = lab[1], "4" = lab[2], "5" = lab[5])
  ) +
  xlab(expression(rho)) +
  ylab("FDR") +
...
results_df <- data.frame(
  stops = c(result_bm[[1]]$stop[result_bm[[1]]$rejects], result_bc[[1]]$stop[result_bc[[1]]$rejects]),
...
p <- ggplot(results_df, aes(stops, color = group)) +
  ...
  geom_vline(
    data = mean_df, aes(xintercept = mean_perm_rej, color = group),
    linetype = "dashed"
  ) +
  scale_colour_manual(
    name = "Strategy", values = c("bm" = col[5], "bc" = col[4]),

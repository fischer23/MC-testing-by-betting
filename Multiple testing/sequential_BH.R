...
sequential_BH=function(true_stat, perm_stat, M, B, peeking_times=(1:B), alpha=0.05, param_bm=0.9, param_bc=10,
  ...
  for(i in peeking_times){
    ...
    #Stop for futility
    consider=setdiff(consider, which(ifelse((1-pbinom(losses,i+1,
                                                      param_bm*alpha*(length(consider)+m_star)/M))/(param_bm*alpha*
    ...
    repeat{
      ...
      for(u in consider){
        ...
        if(time[u]<length(peeking_times_AMT)){
          ac=binom.confint(losses[u], n = peeking_times_AMT[time[u]], 

### Simulates PRDS data and applies different procedures to it
...
# rhos:                Correlation test statistics (vector of different values)
...
plot_generator_PRDS <- function(nsim, nhyp, mu_As, mu_N, pis, rhos, alphas, param_bm, param_bc, param_amt, peeking_times_AMT, B_large, B_small, filename) {
  ### Predefine vectors for power, FDR and mean number of permutations for different procedures
  power_bm <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  FDR_bm <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_mean_bm <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_mean_rej_bm <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_mean_stop_bm <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_median_bm <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_max_bm <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))

  power_bc <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  FDR_bc <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_mean_bc <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_mean_rej_bc <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_mean_stop_bc <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_median_bc <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_max_bc <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))

  power_agg <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  FDR_agg <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_mean_agg <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_mean_rej_agg <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_mean_stop_agg <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_median_agg <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  nperm_max_agg <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))

  power_BH <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  FDR_BH <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))

  power_BH_smallB <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))
  FDR_BH_smallB <- matrix(0, nsim, length(pis) * length(alphas) * length(mu_As) * length(nhyp) * length(rhos))


  count <- 1
  ### Generate PRDS test statistics
  for (M in nhyp) {
    for (rho in rhos) {
      for (alpha in alphas) {
        for (mu_A in mu_As) {
          for (pi_A in pis) {
            ...
            for (j in 1:nsim) {
              ...
              X <- mvrnorm(1, rep(0, M), matrix(rho, ncol = M, nrow = M) + diag(1 - rho, nrow = M, ncol = M))
            ...
            for (k in 1:nsim) {
              ...
              FDR_bm[k, count] <- length(intersect(which(hypo[, k] == 0), results_bm$rejects)) / max(1, length(results_bm$rejects))
